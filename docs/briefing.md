...
### **Níveis de Controle:**
1.  **Menu & Navegação:** Itens de menu no *Sidebar* do Quasar só aparecem se o grupo tiver a permissão `view_menu`.
2.  **Acesso a Telas:** Bloqueio via *Navigation Guards* no Vue Router.
...
4.  **Backend Guard:** O NestJS validará cada requisição via *Guards* de API, impedindo acessos não autorizados mesmo via chamadas diretas (Postman/Insomnia).
...
* **Billing Unit:**
    ...
    2. Taxa de Ativação por Assento — valor único (*one-time fee*) por membro adicional.
* **Paywall:** Bloqueio de escrita e visualização de gráficos para contas inadimplentes ou após expiração do trial (modo *Read-only*).
...
---
*Este documento serve como guia base para o início do desenvolvimento (Sprint 0).*